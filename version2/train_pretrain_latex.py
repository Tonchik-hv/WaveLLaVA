...
from models_latex import VisualToGPTMapping, initialize_special_embs
from dataset_latex import get_dataset, get_collate_function
...
class Model_pl(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # with torch.autocast(device_type="cuda", dtype=torch.float16):
            # logits = self.model(inputs_embeds=embeddings.to(dtype = torch.float16), output_hidden_states=True).get("logits")[:, :-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='./configs_latex/config-pretrain_latex.json')
    ...
    # cfg.pad_id = unk_id # TODO: cfg.pad_id = tokenizer.pad_token_id
    cfg.pad_id = tokenizer.pad_token_id
    ...
    projection = VisualToGPTMapping(3072, cfg.emb_dim).to(dtype=torch.float32)
    ...
    trainer = pl.Trainer(devices=[2,3], max_epochs=cfg.n_epochs, logger = logger, accumulate_grad_batches = cfg.grad_accum, strategy='ddp_find_unused_parameters_true')
    trainer.fit(module)
